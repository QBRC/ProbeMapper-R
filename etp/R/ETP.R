# Framework for the EntrezToProbe (ETP) package. 
# 
# Author: jeffreya
###############################################################################
...
#' @param conn the connection object generated by etp.connect()
#' @returnType list
#' @return a list of the available platforms with their IDs as the row.names
#' @author Jeffrey D. Allen \email{Jeffrey.Allen@@UTSouthwestern.edu}
#' @export
etp.getPlatforms <- function (conn){
...
#' @author Jeffrey D. Allen \email{Jeffrey.Allen@@UTSouthwestern.edu}
#' @export
etp.getGene <- function(conn, entrezID){
...
#' @author Jeffrey D. Allen \email{Jeffrey.Allen@@UTSouthwestern.edu}
#' @export
etp.getProbe <- function(conn, probeID, platformID, probeName){
	...
	if (missing(probeID)){
		...
		if (!missing(platformID) || !missing(probeName)){
			warn("The probeID was provided, so probeName and platformID will be ignored.");
		}
	}
...
#' @param authorityID a number of vector of numbers representing the ID(s) of the authorities (sources) which you want to use to map this probe to genes. At the time of writing, 1 = BLAST, 2 = Vendor, 3 = Bioconductor.
#' @returnType list
#' @return a list containing information about the probe specified and a data.frame detailing the genes with which this probe is associated. There is a normalized [0-1] value representing the strength of that association according to each authority in the database.
#' @author Jeffrey D. Allen \email{Jeffrey.Allen@@UTSouthwestern.edu}
#' @export
etp.getGenesByProbe <- function (conn, probeID, platformID, probeName, authorityID){	
...
#' @param platformID a number or vector of numbers representing the ID(s) of the platforms from which you want to find probes.
#' @param authorityID a number of vector of numbers representing the ID(s) of the authorities (sources) which you want to use to map this gene to probes. At the time of writing, 1 = BLAST, 2 = Vendor, 3 = Bioconductor. 
#' @returnType list
#' @return a list containing information about the gene specified and a data.grame detailing the probes with which this gene is associated. There is a normalized [0-1] value representing the strength of that association according to each authority in the database. 
#' @author Jeffrey D. Allen \email{Jeffrey.Allen@@UTSouthwestern.edu}
#' @export
etp.getProbesByGene <- function (conn, entrezID, platformID, authorityID){
